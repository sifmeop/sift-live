    ...
    <>
      <div>
        ...
        <a href="https://react.dev" target="_blank">
          <img src={reactLogo} className="logo react" alt={content.reactLogo.value} />
        </a>
      </div>
      <h1>{content.title}</h1>
      <div className="card">
...
const LocaleSwitcher: FC = () => {
  ...
  return <button onClick={() => setLocale(Locales.English)}>Change Language to English</button>
}
```
    ...
    <Routes>
      {localeMap(({ locale, urlPrefix }) => (
        <Route
          ...
          path={`${urlPrefix}/*`}
          key={locale}
          element={<IntlayerProvider locale={locale}>{children}</IntlayerProvider>} // Wraps children with locale management
        />
      ))}
    </Routes>
  </BrowserRouter>
...
import { forwardRef, type DetailedHTMLProps, type AnchorHTMLAttributes } from 'react'
import { useLocale } from 'react-intlayer'
...
export const Link = forwardRef<HTMLAnchorElement, LinkProps>(
  ({ href, children, ...props }, ref) => {
    ...
    const hrefI18n = href && !isExternalLink ? getLocalizedUrl(href, locale) : href
