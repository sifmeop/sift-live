...
const FieldErrorContent = ({ errors }: { errors?: Array<{ message?: string } | undefined> }) => {
  if (!errors?.length) return null

  const filteredErrors = errors?.filter((error): error is { message: string } => !!error?.message)
  const uniqueErrors = [...new Map(filteredErrors.map((error) => [error.message, error])).values()]
...
const FieldError = ({
  ...
  return (
    <div
      ...
      {...props}
    >
      {children ?? <FieldErrorContent errors={errors} />}
    </div>
  )
}
